...
@flojoy(deps={"opencv-python": "4.8.1.78"})
def OBJECT_DETECTION(default: Image) -> Image:
    ...
    path = from_cur_dir("yolov3.weights")
...
def from_cur_dir(file):
    return os.path.join(os.path.dirname(__file__), file)


def load_classes():
    with open(from_cur_dir("yolov3.txt"), "r") as f:
...
def detect_object(img_np_array):
    ...
    net = cv2.dnn.readNet(
        from_cur_dir("yolov3.weights"),
        from_cur_dir("yolov3.cfg"),
    )
